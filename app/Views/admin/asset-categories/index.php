<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <a href="/admin/asset-categories/create"
...
href="?search=<?= urlencode($search) ?>&per_page=<?= $perPage ?>&page=<?= $paginator->currentPage() ?>&sort=category_code&direction=<?= ($sort=='category_code' && $direction=='ASC') ? 'DESC':'ASC' ?>">

Code

<?php if($sort=='category_code'): ?>
...
<a class="text-decoration-none text-dark"
href="?search=<?= urlencode($search) ?>&per_page=<?= $perPage ?>&page=<?= $paginator->currentPage() ?>&sort=category_name&direction=<?= ($sort=='category_name' && $direction=='ASC') ? 'DESC':'ASC' ?>">
...
<a class="text-decoration-none text-dark"
href="?search=<?= urlencode($search) ?>&per_page=<?= $perPage ?>&page=<?= $paginator->currentPage() ?>&sort=sort_order&direction=<?= ($sort=='sort_order' && $direction=='ASC') ? 'DESC':'ASC' ?>">
...
<a class="text-decoration-none text-dark"
href="?search=<?= urlencode($search) ?>&per_page=<?= $perPage ?>&page=<?= $paginator->currentPage() ?>&sort=status&direction=<?= ($sort=='status' && $direction=='ASC') ? 'DESC':'ASC' ?>">
...
<a
href="/admin/asset-categories/edit?id=<?= $category['id'] ?>"
